...
namespace qtools.qhierarchy.pcomponent
{
    public class QVisibilityComponent: QBaseComponent
    {
        ...
        public override void draw(GameObject gameObject,  Rect selectionRect, Rect curRect)
        {
            ...
            bool editModeVisibleObjectsContains = isEditModeVisibile(gameObject);
            bool editModeInvisibleObjectsContains = isEditModeInvisibile(gameObject);
        ...
        public override void eventHandler(GameObject gameObject,  Event currentEvent, Rect curRect)
        {
            if (currentEvent.isMouse && currentEvent.button == 0 && curRect.Contains(currentEvent.mousePosition))
            {
                ...
                setVisibility(targetGameObjects, !gameObject.activeSelf, currentEvent.control || currentEvent.command);
                currentEvent.Use();  
            } 
        }

        // PRIVATE
        private bool isEditModeVisibile(GameObject gameObject)
        {
            return false;
        }
        
        private bool isEditModeInvisibile(GameObject gameObject)
        {
            return false;
        }
        
        private void setVisibility(List<GameObject> gameObjects,  bool targetVisibility, bool editMode)
        {
