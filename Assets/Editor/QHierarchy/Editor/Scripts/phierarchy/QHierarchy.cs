    ...
    public class QHierarchy
    {
        ...
        public void hierarchyWindowItemOnGUIHandler(int instanceId, Rect selectionRect)
        {
            try
            {
                ...
                drawComponents(preComponents    , selectionRect, ref curRect, gameObject);
                drawComponents(orderedComponents, selectionRect, ref curRect, gameObject, hideIconsIfThereIsNoFreeSpace, selectionRect.x + gameObjectNameWidth + 7);
                drawComponents(postComponents   , selectionRect, ref curRect, gameObject);
        ...
        private void drawComponents(List<QBaseComponent> components, Rect selectionRect, ref Rect curRect, GameObject gameObject,  bool trim = false, float minX = 50)
        {
            ...
            if (Event.current.type == EventType.Repaint)
            {
                for (int i = 0, n = components.Count; i < n; i++)
                {
                    ...
                    {
                        component.layout(gameObject,  ref rect);
                        if (trim && minX > rect.x)
                        {
                        ...
                        component.draw(gameObject,  selectionRect, rect);
            ...
            else if (Event.current.isMouse)
            {
                for (int i = 0, n = components.Count; i < n; i++)
                {
                    ...
                    if (component.isEnabled())
                    {
                        ...
                        component.eventHandler(gameObject,  Event.current, rect);
                        curRect.Set(rect.x, rect.y, rect.width, rect.height);
                    }
                }
            }
